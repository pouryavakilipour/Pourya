...
void TMVA::Factory::DeleteSingleMethod( TString datasetname )
{
   std::map<TString,MVector*>::iterator itrMap;
   
   itrMap = fMethodsMap.find(datasetname); //////////////////////////////////////////////////////////////////////>>>>>>>>>>>>>>>>>>>>>>>>
...
void TMVA::Factory::TrainSingleMethod(TString datasetname) 
{  
   ...
   std::map<TString,MVector*>:: const_iterator itrMap;
   
   itrMap = fMethodsMap.find(datasetname);
   
      MVector *methods=itrMap->second;
      MVector::iterator itrMethod;
      //std::cout << itrMap->first <<"  ===>>>  "<< methods->size() << std::endl; /////////////////////////////////////////////////////
...
void TMVA::Factory::TestSingleMethod(TString datasetname)
{
...
void TMVA::Factory::EvaluateSingleMethod( TString datasetname )
{
...
float TMVA::Factory::CrossValidate(DataLoader * loader, Types::EMVA theMethod, TString methodTitle, const char *theOption, bool optParams, int NumFolds, bool remakeDataSet, float * rocIntegrals)
{
   ...
   //UInt_t Nthreads = NumFolds;
   UInt_t Nthreads = 2;
   ROOT::EnableThreadSafety();
   ThreadPool pool(Nthreads);
 
   //std::vector<double> ROCs;  
   fSilentFile = true; // to avoid writing on the file from the thread
   if(!optParams){
      auto ROCs = pool.Map([=](Int_t j) -> double { 
	 ...
	 //seedloader->PrepareTrainingAndTestTree(j, TMVA::Types::kTesting);
         ...
         BookMethod(seedloader, theMethod, methodTitle, theOption);
      	 ...
      	 TString datasetname = seedloader->GetName();
         TrainSingleMethod(datasetname);
         //TestSingleMethod(datasetname);
	 
         //EvaluateSingleMethod(datasetname);
	 
	 //std::cout << GetROCIntegral(seedloader->GetName(), methodTitle) << std::endl;
	 std::cout << GetROCIntegral(datasetname, methodTitle) << std::endl;
	 
	 //std::cout<<"I'm thread number : " << j << std::endl;
	...
	// }
         //this->DeleteSingleMethod(foldTitle);
	 
      
        // fMethodsMap.clear();
	 
	 //return (GetROCIntegral(seedloader->GetName(), methodTitle));}, ROOT::TSeqI(Nthreads));
	return 0;}, ROOT::TSeqI(Nthreads));
   }   
   
   //pool.Map(par, ROOT::TSeqI(Nthreads));
...
float TMVA::Factory::CrossValidate(DataLoader * loader, Types::EMVA theMethod, TString methodTitle, const char *theOption, bool optParams, int NumFolds, bool remakeDataSet, float * rocIntegrals, UInt_t nPro)
{
   ...
   TProcPool pool(nPro);
 	 ...
 	 TString foldTitle = methodTitle;
         ...
         TrainSingleMethod(foldTitle);
         TestSingleMethod(foldTitle);
         EvaluateSingleMethod(foldTitle);
	
	 Double_t rocint ;
	 rocint = GetROCIntegral(seedloader->GetName(), methodTitle);
	...
	return rocint;}, ROOT::TSeqI(NumFolds));
...
float TMVA::Factory::CrossValidate(DataLoader * loader, Types::EMVA theMethod, TString methodTitle, const char *theOption, bool optParams, int NumFolds, bool remakeDataSet, float * rocIntegrals)
{
   ...
   UInt_t Nthreads = NumFolds;
   TProcPool pool(Nthreads);
   fSilentFile = true; // to avoid writing on the file from the thread
   std::vector<double> ROCs(Nthreads);  

      auto CVP = [=](Int_t j) -> double { 
	 ...
	 Double_t rocint = GetROCIntegral(seedloader->GetName(), methodTitle);
  	 TMVA::MethodBase * smethod = dynamic_cast<TMVA::MethodBase*>(fMethodsMap[seedloader->GetName()][0][0]);
         TMVA::ResultsClassification * sresults = (TMVA::ResultsClassification*)smethod->Data()->GetResults(smethod->GetMethodName(), Types::kTesting, Types::kClassification);
         sresults->Clear();
         sresults->Delete();
   ...
   ROCs = pool.Map(CVP, ROOT::TSeqI(Nthreads));
...
void TMVA::Factory::CrossValidatePlotROC(DataLoader * loader, Types::EMVA theMethod, TString methodTitle, const char *theOption, bool optParams, int NumFolds, bool remakeDataSet)
{
   ...
   if(!optParams){
	 ...
	 title += j+1;
         ...
         EvaluateAllMethods();
	 TGraph *g = new TGraph();
	 g = GetROCCurve(seedloader->GetName(), "BDT", true);
	 g->SetLineColor(j+1);
	 g->SetLineWidth(3);
	 g->SetTitle(title);
	 mg->Add(g);
   ...
   c->BuildLegend(0.15,0.7,0.3,0.4);
}
